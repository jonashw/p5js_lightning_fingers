new p5((s) => {
  ...
  const lightningColors = Array(55)
    .fill()
    .map((_, i) => s.color(200 + i, 255, 100));
  ...
  let monoSynth;
  let oscillators = [];
  const oscillatorWaves = new CircularArray([
    "triangle",
    "sine",
    "sawtooth",
    "square",
  ...
  s.setup = () => {
    ...
    */
    let freqs = [
      ...
      466.1637615180899, 587.3295358348151, 880, 1046.5022612023945,
      1318.5102276514797, 1567.981743926997, 1864.6550460723597,
      2349.31814333926, 3520, 4186.009044809578, 5274.040910605919,
      6271.926975707989, 7458.620184289437, 9397.272573357044, 14080,
      16744.036179238312, 21096.16364242367, 25087.70790283195,
      29834.480737157748, 37589.09029342818, 56320,
    ];
    oscillators = Array(20)
      .fill()
      .map((_, i) => {
    ...
    lightningFactors = new CircularArray([
      dd * 50,
      dd * 75,
      dd * 100,
      dd * 200,
  ...
  s.draw = () => {
    ...
    centerDiameter = updating
      ? circleDiameter
      : circleDiameter * (1.5 + exitingTouchCircles.length / 10);
    if (updating) {
  ...
  const setFirstNOscillatorsPlaying = (n) => {
    for (var i = 0; i < oscillators.length; i++) {
      ...
      if (n <= i) {
        ...
        if (!osc.started) {
          //starting an already started osc creates an audible 'jerk'
          osc.start();
        }
  ...
  s.touchStarted = (e) => {
    ...
    fingers = s.touches;
    console.log(
      `touch started (total touches: ${s.touches.length}): `,
      touch.id
    );
    return false; /* This is to prevent pinch-zooming on touch devices: */
  };

  let shakeSounds = ["C3", "D3", "E3", "F3", "G3", "A3", "B3"];
  const playShakeSound = () => {
    let time = 0;
    let dur = 1 / 6;
    let vel = 0.25;
    let note = shakeSounds[Math.floor(shakeSounds.length * Math.random())];
    monoSynth.play(note, vel, time, dur);
  };

  const shake = () => {
    playShakeSound();
    if (exitingTouchCircles.length === 0) {
      return;
    }
    let c = s.createVector(s.windowWidth / 2, s.windowHeight / 2);
    for (let tc of exitingTouchCircles) {
      let x = Math.floor(Math.random() * s.width);
      let y = Math.floor(Math.random() * s.height);
      let position = s.createVector(x, y);
      let heading = p5.Vector.sub(c, position);
      tc.heading = heading;
      tc.position = position;
      console.log({ position, tc, c });
    }
  };
  ...
  s.touchEnded = (e) => {
    ...
    let touchesToExit = completedTouches.filter(
      (t) => !circleContainsPoint(center, lightningFactors.getCurrent(), t)
    );
    beginExitOfTouches(touchesToExit);
    console.log(
      "finished touches:",
      completedTouches.map((t) => t.id).join(", "),
      completedTouches.map((t) => `x:${t.x},y:${t.y}`)
    );
    return false; /* This is to prevent pinch-zooming on touch devices: */
  };
  ...
  s.keyTyped = (e) => {
    ...
    if (key === "l") {
      ...
      console.log(
        "switched lightningFactor to:",
        lightningFactors.getCurrent()
      );
    }
    if (key === "u") {
      updating = !updating;
    ...
    if (key === " ") {
      console.log("emulating shake gesture");
      shake();
    }
  };
